...
import { Title, Text, Button, Flex, Badge, ProgressBar, TextInput } from "@tremor/react";
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {activeTab === "documents" && (
            <PremiumGlassCard className="p-6">
              ...
              <div className="mt-8 space-y-4">
                <Text className="font-medium text-text-primary dark:text-white">Documents récents</Text>
                {[
                  ...
                  <div key={i} className="flex items-center justify-between p-4 bg-black/5 dark:bg-white/5 rounded-xl border border-black/5 dark:border-white/5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <Text className="font-medium text-text-primary dark:text-white">{doc.name}</Text>
                        <Text className="text-xs text-text-secondary dark:text-white/40">{doc.size}</Text>
                      </div>
                    </div>
            ...
            <PremiumGlassCard className="p-6">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <Text className="text-xs text-text-secondary dark:text-white/40">L'agent lira toutes les pages publiques et indexera le contenu.</Text>
                </div>

                <div className="mt-8 space-y-4">
                  <Text className="font-medium text-text-primary dark:text-white">Sites indexés</Text>
                  <div className="p-4 bg-black/5 dark:bg-white/5 rounded-xl border border-black/5 dark:border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <Text className="font-medium text-text-primary dark:text-white">https://www.k-solutions.cd</Text>
                        <Text className="text-xs text-text-secondary dark:text-white/40">Dernier scan: il y a 2 jours • 42 pages</Text>
                      </div>
                    </div>
            ...
            <PremiumGlassCard className="p-6">
              ...
              <div className="space-y-6">
                <div className="p-6 border border-dashed border-black/10 dark:border-white/10 rounded-xl space-y-4">
                  <div className="space-y-2">
                    <Text className="font-medium text-text-primary dark:text-white">Question</Text>
                    <TextInput placeholder="ex: Quels sont vos horaires d'ouverture ?" />
                  </div>
                  <div className="space-y-2">
                    <Text className="font-medium text-text-primary dark:text-white">Réponse</Text>
                ...
                <div className="mt-8 space-y-2">
                  <Text className="font-medium text-text-primary dark:text-white">Questions enregistrées (12)</Text>
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="p-4 bg-black/5 dark:bg-white/5 rounded-xl border border-black/5 dark:border-white/5">
                      <Flex>
                        <Text className="font-bold text-text-primary dark:text-white">Question {i}</Text>
                        <button className="text-gray-400 hover:text-red-500"><Trash2 size={16} /></button>
                      </Flex>
                      <Text className="mt-1 text-sm text-text-secondary dark:text-white/60">Contenu de la réponse pré-enregistrée pour l'agent.</Text>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 {[
                   ...
                   <div key={i} className="p-6 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5 flex flex-col gap-4">
                      ...
                      <div>
                         <Text className="font-bold text-text-primary dark:text-white">{app.name}</Text>
                         <Text className="text-xs text-text-secondary dark:text-white/40">Synchronisez vos fichiers automatiquement.</Text>
                      </div>
        ...
        <div className="space-y-8">
           <PremiumGlassCard className="bg-blue-600 dark:bg-blue-600 border-none p-6">
              ...
              <div className="mt-6 space-y-4">
                 ...
                 <Button color="blue" className="w-full border-white/20">Sauvegarder</Button>
              </div>
           </PremiumGlassCard>
           ...
           <PremiumGlassCard className="p-6">
              ...
              <div className="mt-6 space-y-6">
                 <div>
                    <Flex>
                       <Text className="text-text-secondary dark:text-white/60">Espace Utilisé</Text>
                       <Text className="text-text-primary dark:text-white font-bold">12.4 MB / 1 GB</Text>
                    </Flex>
