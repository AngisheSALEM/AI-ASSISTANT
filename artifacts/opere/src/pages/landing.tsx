...
import {
  Zap, MessageSquare, ShieldCheck, Headphones, Stethoscope, Building2, ArrowRight
} from "lucide-react";
    ...
    <div className="flex flex-col min-h-screen text-white bg-transparent">
      <header className="px-6 h-20 flex items-center border-b border-white/5 backdrop-blur-md sticky top-0 z-50">
        <Link href="#" className="flex items-center justify-center group">
          <motion.div whileHover={{ rotate: 15 }} className="p-2 bg-white/10 rounded-xl">
            <Zap className="h-6 w-6 text-white" />
          </motion.div>
          <span className="ml-3 text-xl font-bold font-fraunces tracking-tighter">Opere</span>
        </Link>
        <nav className="ml-auto flex gap-8">
          <a className="text-sm font-medium text-white/60 hover:text-white transition-colors" href="#features">Fonctionnalités</a>
          <a className="text-sm font-medium text-white/60 hover:text-white transition-colors" href="#pricing">Tarifs</a>
      ...
      <main className="flex-1">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}>
                  <Link href="/register" className="inline-flex h-14 items-center justify-center rounded-full bg-white px-10 text-sm font-bold text-black shadow-[0_0_20px_rgba(255,255,255,0.3)] transition-colors hover:bg-gray-200">
                    Créer mon compte
                  </Link>
                </motion.div>
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}>
                  <a href="#catalog" className="inline-flex h-14 items-center justify-center rounded-full border border-white/10 bg-white/5 px-10 text-sm font-bold text-white backdrop-blur-md transition-all hover:bg-white/10">
                    Voir le catalogue
                  </a>
                </motion.div>
              </div>
        ...
        <section id="catalog" className="w-full py-24 md:py-32">
          <div className="container px-4 md:px-6 mx-auto">
            ...
            >
              {[
                { icon: Headphones, title: "Support Client", desc: "Répond instantanément aux questions de vos clients et résout les problèmes courants.", price: "50 crédits/mois" },
                { icon: Stethoscope, title: "Secrétaire Médical", desc: "Gère vos rendez-vous et répond aux questions administratives de vos patients.", price: "70 crédits/mois" },
                { icon: Building2, title: "Commercial Immobilier", desc: "Qualifie vos prospects et présente votre catalogue de biens 24h/24.", price: "100 crédits/mois" }
              ].map((job, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
              <div className="space-y-12">
                ...
                <ul className="space-y-8">
                  {[
                    { icon: Zap, title: "Intégration Native WhatsApp", desc: "Connectez votre propre numéro et laissez l'IA répondre à vos clients en temps réel." },
                    { icon: MessageSquare, title: "Multimodalité & Voix", desc: "Vos agents peuvent envoyer et recevoir des messages vocaux avec une fluidité humaine." },
                    { icon: ShieldCheck, title: "Sécurité des Données", desc: "Chaque organisation dispose d'une base de connaissances isolée, sécurisée et cryptée." }
                  ].map((f, i) => (
          ...
          <div className="container px-4 md:px-6 mx-auto text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <PremiumGlassCard key={i} className={(plan as any).popular ? "border-white/30 scale-105 z-10" : "border-white/10"}>
                  <div className="p-10 flex flex-col items-center h-full">
                    {(plan as any).popular && (
                      <div className="mb-6 px-4 py-1 bg-white text-black text-[10px] font-black uppercase tracking-widest rounded-full">Populaire</div>
                    )}
                    ...
                    <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="w-full">
                      <Link href="/register" className={`w-full py-4 rounded-full font-bold text-sm transition-all flex items-center justify-center gap-2 ${(plan as any).popular ? 'bg-white text-black' : 'bg-white/10 text-white hover:bg-white/20 border border-white/10'}`}>
      ...
      <footer className="flex flex-col gap-6 sm:flex-row py-12 w-full shrink-0 items-center px-10 border-t border-white/5 text-white/40">
        ...
        <nav className="sm:ml-auto flex gap-8">
          <a className="text-xs hover:text-white transition-colors" href="#">Conditions</a>
          <a className="text-xs hover:text-white transition-colors" href="#">Confidentialité</a>
        </nav>
      </footer>
    </div>
  );
